...
const Game = ({ onResult, type = "standard", reset = false, onGameChange }) => {
  const standardGame = ["rock", "paper", "scissors"];
  const advancedGame = ["rock", "paper", "scissors", "lizard", "spock"];
  const [options, setOptions] = useState(
    type == "standard" ? standardGame : advancedGame
  );
  ...
  return chosen !== null ? (
    <div className="relative flex flex-col space-y-20 lg:space-y-0">
      <div className="flex items-center justify-between flex-wrap">
        <div className="flex flex-col items-center md:flex-col-reverse lg:absolute lg:left-0 lg:top-0 lg:-translate-x-1/2 lg:-translate-y-1/2">
          <Disc
            ...
            winner={result == "win"}
          />
          <h3 className="mt-6 md:mt-0 md:mb-6 uppercase text-white tracking-widest">
            You Picked
          </h3>
        </div>
      ...
      <div
        className={`opacity-0 ${
          result != null && "animate-scale-in"
        } flex flex-col space-y-6 items-center lg:absolute lg:w-full lg:left-0  lg:-top-12`}
      >
        <h2 className="text-white uppercase text-4xl">You {result}</h2>
        <button
          onClick={handlePlayAgain}
          className="block bg-white text-dark rounded uppercase h-12 w-55 text-center hover:text-rock-dark"
        >
          Play Again
        </button>
        <button
          onClick={() =>
            type == "standard"
              ? onGameChange("advanced")
              : onGameChange("standard")
          }
          className="uppercase text-white rounded-lg h-11 inline-block px-9 hover:text-lizard-light"
        >
          or Play {type == "standard" ? "Lizard Spock" : "Rock Paper Scissors"}
        </button>
      </div>
    </div>
  ) : (
    <div className="discs relative mx-auto flex flex-wrap justify-center items-center">
      ...
      <img
        ...
        alt=""
        className={`${hide && "animate-fade-out"}`}
      />
    </div>
  );
};
