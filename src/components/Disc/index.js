...
const Disc = ({
    ...
    return (
        <div className={`z-10 ${total ? `disc-${index}of${total}` : 'relative'}`}>
            ...
            {
                option != null && <div className="relative z-20 animate-disc-in">
                    <div className={`rounded-full ${winner && 'animate-disc-winner-sm md:animate-disc-winner-lg'}`}>
                        <button disabled={disabled} onClick={() => onClick(option)} title={`Play ${option}`} className={`relative z-10 bg-${option}-light rounded-full w-33 h-32 p-4 ${type == 'standard' && 'md:w-49 md:h-48 md:p-6'} ${size === 'lg' && type == 'standard' ? 'lg:w-74 lg:h-72 lg:p-8' : ''} flex items-stretch justify-self-stretch shadow-disc-${option}-sm md:shadow-disc-#{option}-md transition-all hover:rotate-12 hover:scale-110 active:scale-95`}>
